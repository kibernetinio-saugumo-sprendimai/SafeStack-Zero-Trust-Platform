...
def serve(db_path='zero-trust.db', host='127.0.0.1', port=8787, allow_remote=False, api_token=None):
    if host not in ('127.0.0.1', 'localhost', '::1') and not allow_remote:
        raise ValueError('remote binding requires explicit allow_remote=True')
    token = api_token or os.environ.get('SAFESTACK_API_TOKEN')
    if allow_remote and not token:
        raise ValueError('remote binding requires SAFESTACK_API_TOKEN')
    store=Store(db_path)
    class Handler(BaseHTTPRequestHandler):
        def send(self, body, content_type):
            raw=body.encode(); self.send_response(200); self.send_header('Content-Type',content_type); self.send_header('Content-Length',str(len(raw))); self.send_header('X-Content-Type-Options','nosniff'); self.send_header('Cache-Control','no-store'); self.send_header('Content-Security-Policy', "default-src 'none'; style-src 'unsafe-inline'"); self.end_headers(); self.wfile.write(raw)
        def do_GET(self):
